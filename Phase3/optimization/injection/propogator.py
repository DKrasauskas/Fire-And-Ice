...
fn.fixed_step_size = 100.0
fn.cube = 500 #e6 # 1000km
fn.REFERENCE_FRAME = 1
...
for i in range(0, 10):
    ...
    time = [times[i]]
    output1, ground_track             = fn.perform_flyby(time,  bodies_to_create, bodies, body_settings,DURATION, n_steps= 10)
...
for i, output1 in enumerate(output):
    ...
    ax1.plot(output1[1][0][::100], output1[1][1][::100], output1[1][2][::100], color="cyan")
    # ax1.plot(output1[2][0], output1[2][1], output1[2][2])


bod_names = [ "Jupiter", "Ganymede", "Callisto", "Europa", "Io"]

output1 = fn.plot_jovian_system(0, bodies_to_create, bodies, body_settings, 60 *60 *24 * 30, n_steps = 1)
ax1.scatter(0, 0, 0)
for item, i in enumerate(output1):
     ax1.plot(i[0], i[1], i[2], label=bod_names[item])
