...
M = 0#eccentricity * np.sinh(F) - F# - 0.01
mean_anomaly_at_j2000 = M + 0.01
arg_of_periapsis = np.radians(140.0)
...
# LAN         = -1.4933 deg
    inclination      = np.radians(0.5657)
    lan              = np.radians(-1.4933)
    jupiter_gravitational_parameter = 1.266e17
    semi_major_axis =  70.46 * 78420000
    eccentricity = 0.9852

    # --- Mean Anomaly ---
    mean_motion = np.sqrt(jupiter_gravitational_parameter / abs(semi_major_axis)**3)
    M = 0 #mean_anomaly_at_j2000# - 0.01# + mean_motion * current_time
