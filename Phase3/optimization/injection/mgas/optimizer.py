...
def optimize(bodies, central_body, departure_orbit, arrival_orbit, transfer_body_order, minimum_pericenters, verbose_output = False):
    ...
    departure_date_lb = DateTime(1951, 7, 4, 3, 17, 0).to_epoch()
    departure_date_ub = DateTime(1951, 7, 4, 12, 17, 0).to_epoch()
    ...
    for i in range(len(legs_tof_lb)):
        legs_tof_lb[i] = 0 * constants.JULIAN_DAY
        legs_tof_ub[i] = 7 *  constants.JULIAN_DAY
        
    optimizer = hp.TransferTrajectoryProblem(
    ...
    if verbose_output:
        ...
        plt.close()
    au = 6.9e7
    node_times, leg_free_parameters, node_free_parameters = hp.convert_trajectory_parameters(
...
if __name__ == "__main__":
    list_of_orders = [
         ["target_orbit",  "Ganymede", "Callisto"],
         ["target_orbit", "Ganymede", "Callisto"],
         ["target_orbit", "Ganymede", "Callisto"],
         ["target_orbit", "Ganymede", "Callisto"],
    ]
    ...
    bodies, central_body, departure, arrival = setup_state(180)
    optimize(bodies, central_body, departure, arrival, list_of_orders[0], minimum_pericenters, True)
